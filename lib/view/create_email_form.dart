...
class EmailTaskCreationView extends StatelessWidget {
  EmailTaskCreationView({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CupertinoNavigationBar(
        ...
        middle: const Text("Create Email Task"),
        backgroundColor: Colors.white,
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            _buildTextField("Recipient Email", recipientController),
            const SizedBox(height: 10),
            _buildTextField("Subject", subjectController),
            const SizedBox(height: 10),
            _buildTextField("Email Body", bodyController, isMultiline: true),
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          // Validate and create email task
          if (controller.validateFields2(recipientController.text,
              subjectController.text, bodyController.text)) {
            // You might want to update this if there's a specific method to create email tasks
            controller.createEmailTask(recipientController.text,
                subjectController.text, bodyController.text);
            // Show snackbar for task creation success
            Get.snackbar(
              "Success",
              "Email task for ${subjectController.text} created successfully at ${controller.selectedTime.value.format(context)}",
